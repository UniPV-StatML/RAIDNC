...
3. **Normalization** — K-mer counts are normalized by the total count per sequence, converting raw counts into relative frequencies. This makes the features comparable across sequences of different lengths.
...
| `--model_dir` | Directory containing `rf_model.joblib` and `kmer_vectorizer.joblib` | Script's own directory |
| `--helpme` | Show detailed usage information and exit | |
...
| Column | Description |
|---|---|
| `Transcript_ID` | Sequence identifier from the FASTA header |
